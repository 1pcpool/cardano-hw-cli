...
const TrezorCryptoProvider: () => Promise<CryptoProvider> = async () => {
  ...
  const showAddress = async (
    paymentPath: BIP32Path, _paymentScriptHash: string, stakingPath: BIP32Path, _stakingScriptHash: string, address: Address,
  ): Promise<void> => {
    const { addressType, networkId, protocolMagic } = getAddressAttributes(address)
    if (addressType in [AddressType.BASE_PAYMENT_KEY_STAKE_SCRIPT, AddressType.BASE_PAYMENT_SCRIPT_STAKE_KEY, AddressType.BASE_PAYMENT_SCRIPT_STAKE_SCRIPT]) {
